...
def final(request, *args, **kwargs):
    if request.method=="POST":
        ...
        student.save()
        send_mail('Recommendation Letter', 'congratulation you recieved recommendation letter ,http://127.0.0.1:8000/', 'christronaldo9090909@gmail.com', [student.email], fail_silently=False)
    ...
    if request.method=="POST":
        ...
        if User.objects.filter(username__exact=usern).exists():
            ...
            if user is not None:
                ...
                #yo request cha ki check garnalaii banako
                dataharu = StudentData.objects.filter( professor__name=sir_name)

                #yo chai ayeko request dekhauna
                std_ko_dataharu = StudentData.objects.filter( professor__name=sir_name,is_generated=False)
                ...
                std_dataharu=serializers.serialize("json",StudentData.objects.filter( professor__name=sir_name,is_generated=True))
                
                return render(request, 'Teacher.html',{'student_list':dataharu,'check_value':check_value,'std_dataharu':std_dataharu,'std_ko_dataharu':std_ko_dataharu})
        ...
        # No backend authenticated the credentials
            ...
            messages.error(request, 'Please Fill the Password too')
            return render(request, 'loginTeacher.html')
    return render(request, 'loginTeacher.html')
...
def otp(request):
    ...
    if request.method=="POST"  :
        ...
        teacher_naam=Usernaam.replace("_"," ")
        
        if TeacherInfo.objects.filter(name=teacher_naam).exists():
            teacherkonam=TeacherInfo.objects.get(name=teacher_naam)
            OTP_value=request.COOKIES.get('OTP_value')
           
            send_mail('OTP ', 'Your OTP for Recoomendation Letter is '+ str(OTP_value),'christronaldo9090909@gmail.com',  [teacherkonam.email], fail_silently=False)
              
            response= render(request, 'otp.html',{'teacherkonam':teacherkonam,'OTP_value':OTP_value})
            #making cookies to store and send them to other view page
            
            response.set_cookie('teacher_ko_naam',teacherkonam)
